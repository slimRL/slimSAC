...
# Parameters used when creating the replay memory.
...
class ReplayBufferTest(parameterized.TestCase):
    ...
    def testSampleTransitionBatch(self):
        ...
        expected_states = [rb.memory[rb.sampling_distribution.index_to_key[i]].state for i in indices]
        expected_next_states = [rb.memory[rb.sampling_distribution.index_to_key[i]].next_state for i in indices]
