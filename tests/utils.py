...
from slimsac.sample_collection.replay_buffer import ReplayElement
...
class Generator:
    ...
    def sample(self, key: jax.random.PRNGKey) -> ReplayElement:
        state = jax.random.uniform(key, self.observation_dim)
        action = jax.random.uniform(key, (self.n_actions,), minval=-1, maxval=1)
